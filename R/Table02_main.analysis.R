modelsummary_w1mi<- function(data) {
  ...
  wb2 <- with(data,
              coeftest(
                lm(covid_SWEMWBS ~ existing_SWEMWBS+
                     cov_symp+ethnicity+region+
                     residence*male+
                     moved+
                     economic_activity+
  ...
  cm <- c("(Intercept)" = 'Constant',#1
          ...
          "movedYes" = "Living Arrangements Changed since Mar ‘20",#6
  ...
  table <- table %>%
    ...
    tab_source_note(source_note = md("*Notes*: Weighted OLS regression estimates, with cluster robust standard errors in parentheses. Significance: '+' p < 0.1, '*' p < 0.05, '**' p < 0.01. All models control for country of residence (England, Scotland, Wales, Northern Ireland), ethnicity (non-white), and experience of COVID-19 symptoms. The unweighted sample is N = 2,578 and missing values are filled with multiple imputation of chained equations."))
