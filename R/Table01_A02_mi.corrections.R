...
descriptives_impw1_binary <- function(df, string){
  ...
  economic_wpct <- with(
    df,by(df,
          .imp, 
          function(x) c(
            weights::wpct(x$economic_activity,
                          weight = x$CW1_COMBWT,
                          na.rm=TRUE)
          )))
  economic_wpct <- round(100*(
    Reduce("+",economic_wpct)/length(economic_wpct)),
    digits = 1)
  economic_wpct <- as.data.frame(economic_wpct) %>%
    mutate(id = c("In University", "Employed", "Furloughed", "In Training/Edu", "Other Inactive", "Unemployed")) %>%
    rename(wpct = economic_wpct)
  
  economic_count <- df %>%
    filter(!is.na(economic_activity)) %>%
    group_by(.imp, economic_activity) %>%
    summarise(n = n(), .groups = 'drop') %>%
    group_by(economic_activity) %>%
    summarise(count = mean(n), .groups = 'drop') %>%
    mutate(count = round(count, digits = 0)) %>%
    rename(n = count) %>%
    mutate(id = economic_activity)
  ...
  economic <- economic_wpct %>%
    full_join(economic_count, by = c("id")) %>%
    select(-id) %>%
    rename(term = economic_activity) %>%
    rename(estimate = n,
           std.error = wpct) %>%
    mutate(summary = paste(estimate,"(",std.error,")"),
           dataset = string)
  
  rbind(step,single,income,education,economic)
...
descriptives_impw2_binary <- function(df, string){
  ...
  economic_wpct <- with(
    df,by(df,
          .imp, 
          function(x) c(
            weights::wpct(x$CW2_Economic.Activity,
                          weight = x$CW2_COMBWT,
                          na.rm=TRUE)
          )))
  economic_wpct <- round(100*(
  ...
  economic_wpct <- as.data.frame(economic_wpct) %>%
    mutate(id = c("In Training/Edu", "Employed", "Furloughed", "Other Inactive", "Unemployed")) %>%
    rename(wpct = economic_wpct)
  
  economic_count <- df %>%
    filter(!is.na(CW2_Economic.Activity)) %>%
    group_by(.imp, CW2_Economic.Activity) %>%
    summarise(n = n(), .groups = 'drop') %>%
    group_by(CW2_Economic.Activity) %>%
    summarise(count = mean(n), .groups = 'drop') %>%
    mutate(count = round(count, digits = 0)) %>%
    rename(n = count) %>%
    mutate(id = CW2_Economic.Activity)
  ## -- Education -- ##
  edu_wpct <- with(
    df,by(df,
          .imp, 
  ...
  economic <- economic_wpct %>%
    full_join(economic_count, by = c("id")) %>%
    select(-id) %>%
    rename(term = CW2_Economic.Activity) %>%
    rename(estimate = n,
           std.error = wpct) %>%
    mutate(summary = paste(estimate,"(",std.error,")"),
           dataset = string)
  
  rbind(step,single,income, economic, education)
}
...
descriptives_impw3_binary <- function(df, string){
  ...
  economic_wpct <- with(
    df,by(df,
          .imp, 
          function(x) c(
            weights::wpct(x$CW3_Economic.Activity,
                          weight = x$CW3_COMBWT,
                          na.rm=TRUE)
          )))
  economic_wpct <- round(100*(
  ...
  economic_count <- df %>%
    filter(!is.na(CW3_Economic.Activity)) %>%
    group_by(.imp, CW3_Economic.Activity) %>%
    summarise(n = n(), .groups = 'drop') %>%
    group_by(CW3_Economic.Activity) %>%
    summarise(count = mean(n), .groups = 'drop') %>%
    mutate(count = round(count, digits = 0)) %>%
    rename(n = count) %>%
    mutate(id = CW3_Economic.Activity)
  # -- Education -- #
  edu_wpct <- with(
    df,by(df,
          .imp, 
  ...
  economic <- economic_wpct %>%
    full_join(economic_count, by = c("id")) %>%
    select(-id) %>%
    rename(term = CW3_Economic.Activity) %>%
    rename(estimate = n,
           std.error = wpct) %>%
    mutate(summary = paste(estimate,"(",std.error,")"),
           dataset = string)
