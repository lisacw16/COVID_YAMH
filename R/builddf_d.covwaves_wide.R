    ...
    assign(paste0("cov_wave", i),
           load_sav_file_function(data_files[i]) %>%
             ...
             mutate(mobility = as.factor(
               case_when(
                 if_all(matches("^.*_COURSERET$"), 
                        is_yes) ~ 
                   "Returning to Course",
                 if_all(matches('^.*EDUOFFERINTENT$'), 
                        is_EDUOFFERINTENT) ~
                   "Returning to Course",
                 if_all(matches('^.*WRKLOCATIOND'), 
                        is_WRKLOCATIOND) ~
                   "Working in Person",
                 .default = "Unspecified"))) %>%
  ...
  full_join(cm_der7, cov_wave1, by = c("MCSID")) %>%
    ...
    mutate(CW2_moved = as.factor(
      case_when(
        ...
        is.na(CW2_moved) & !is.na(CW1_moved) ~ CW1_moved,
        .default = "No"
      )),
      CW3_moved = as.factor(
        case_when(
          !is.na(CW3_moved) ~ CW3_moved,
          is.na(CW3_moved) & !is.na(CW2_moved) ~ CW2_moved,
          is.na(CW2_moved) & !is.na(CW1_moved) ~ CW1_moved,
          .default = "No"
        )
      ))
...
## For-Loop Sub-Functions (use for COVID waves)
### Changed Living Arrangement
...
factor.econ <-  function(x) {
  relevel(factor(x, 
         ...
         labels = 
           ...
           "Other Inactive", 
           "In Training/Edu", 
           "Other Inactive")), ref = "In Training/Edu")
}
### Covid Symptoms
